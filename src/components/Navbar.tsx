...
const Navbar = () => {
  ...
  return (
    <div className="w-full h-20 bg-white border-b-[1px] border-b-gray-400 sticky top-0 z-50">
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-white border-t border-gray-400 md:hidden">
          <div className="flex flex-col items-center py-4">
            {navBarList.map((item) => (
              <Link
                href={item?.link}
                key={item?.link}
                className={`block w-full text-center py-2 text-gray-600 hover:bg-gray-200 ${
                  pathname === item?.link && "text-gray-950 font-bold"
                }`}
            ...
            {session?.user && (
              <button
                ...
                className="block w-full text-center py-2 text-gray-500 hover:bg-red-200"
              >
                Logout
              </button>
            )}
